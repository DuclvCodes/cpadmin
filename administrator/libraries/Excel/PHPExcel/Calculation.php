...
if (!defined('CALCULATION_REGtkp_CELLREF')) {
    //    Test for support of \P (multibyte options) in PCRE
    if (defined('PREG_BAD_UTF8_ERROR')) {
        //    Cell reference (cell or range of cells, with or without a sheet reference)
        define('CALCULATION_REGtkp_CELLREF', '((([^\s,!&%^\/\*\+<>=-]*)|(\'[^\']*\')|(\"[^\"]*\"))!)?\$?([a-z]{1,3})\$?(\d{1,7})');
        //    Named Range of cells
        define('CALCULATION_REGtkp_NAMEDRANGE', '((([^\s,!&%^\/\*\+<>=-]*)|(\'[^\']*\')|(\"[^\"]*\"))!)?([_A-Z][_A-Z0-9\.]*)');
    } else {
        //    Cell reference (cell or range of cells, with or without a sheet reference)
        define('CALCULATION_REGtkp_CELLREF', '(((\w*)|(\'[^\']*\')|(\"[^\"]*\"))!)?\$?([a-z]{1,3})\$?(\d+)');
        //    Named Range of cells
        define('CALCULATION_REGtkp_NAMEDRANGE', '(((\w*)|(\'.*\')|(\".*\"))!)?([_A-Z][_A-Z0-9\.]*)');
    }
}
...
class PHPExcel_Calculation
{
    ...
    /** Regular Expressions        */
    //    Numeric operand
    const CALCULATION_REGtkp_NUMBER        = '[-+]?\d*\.?\d+(e[-+]?\d+)?';
    //    String operand
    const CALCULATION_REGtkp_STRING        = '"(?:[^"]|"")*"';
    //    Opening bracket
    const CALCULATION_REGtkp_OPENBRACE    = '\(';
    //    Function (allow for the old @ symbol that could be used to prefix a function, but we'll ignore it)
    const CALCULATION_REGtkp_FUNCTION    = '@?([A-Z][A-Z0-9\.]*)[\s]*\(';
    //    Cell reference (cell or range of cells, with or without a sheet reference)
    const CALCULATION_REGtkp_CELLREF    = CALCULATION_REGtkp_CELLREF;
    //    Named Range of cells
    const CALCULATION_REGtkp_NAMEDRANGE    = CALCULATION_REGtkp_NAMEDRANGE;
    //    Error
    const CALCULATION_REGtkp_ERROR        = '\#[A-Z][A-Z0_\/]*[!\?]?';
    ...
    public static function wrapResult($value)
    {
        if (is_string($value)) {
            ...
            if (preg_match('/^'.self::CALCULATION_REGtkp_ERROR.'$/i', $value, $match)) {
    ...
    private function _parseFormula($formula, PHPExcel_Cell $pCell = null)
    {
        ...
        $regexpMatchString = '/^('.self::CALCULATION_REGtkp_FUNCTION.
                               '|'.self::CALCULATION_REGtkp_CELLREF.
                               '|'.self::CALCULATION_REGtkp_NUMBER.
                               '|'.self::CALCULATION_REGtkp_STRING.
                               '|'.self::CALCULATION_REGtkp_OPENBRACE.
                               '|'.self::CALCULATION_REGtkp_NAMEDRANGE.
                               '|'.self::CALCULATION_REGtkp_ERROR.
                             ')/si';
...
//echo 'Element is a Closing bracket', PHP_EOL;
                ...
                $d = $stack->last(2);
                if (preg_match('/^'.self::CALCULATION_REGtkp_FUNCTION.'$/i', $d['value'], $matches)) {    //    Did this parenthesis just close a function?
...
//echo 'Element is a Function argument separator', PHP_EOL;
                ...
                $d = $stack->last(2);
                if (!preg_match('/^'.self::CALCULATION_REGtkp_FUNCTION.'$/i', $d['value'], $matches)) {
                    return $this->raiseFormulaError("Formula Error: Unexpected ,");
                }
                $d = $stack->pop();
...
//                echo 'Element with value '.$val.' is an Operand, Variable, Constant, String, Number, Cell Reference or Function<br />';
                ...
                if (preg_match('/^'.self::CALCULATION_REGtkp_FUNCTION.'$/i', $val, $matches)) {
...
//                    echo 'Element '.$val.' is a Function<br />';
                ...
                } elseif (preg_match('/^'.self::CALCULATION_REGtkp_CELLREF.'$/i', $val, $matches)) {
//                    echo 'Element '.$val.' is a Cell reference<br />';
                    ...
                    if ($testPrevOp['value'] == ':') {
                        ...
                        if ($matches[2] == '') {
                            ...
                            preg_match('/^'.self::CALCULATION_REGtkp_CELLREF.'$/i', $startCellRef, $startMatches);
                            if ($startMatches[2] > '') {
...
//                echo 'Possible Intersect Operator<br />';
                if (($expectingOperator) && (preg_match('/^'.self::CALCULATION_REGtkp_CELLREF.'.*/Ui', substr($formula, $index), $match)) &&
...
//                    echo 'Token is a percentile operator<br />';
            ...
            } elseif (preg_match('/^'.self::CALCULATION_REGtkp_CELLREF.'$/i', $token, $matches)) {
                $cellRef = null;
...
//                            echo '$cellRef='.$cellRef.' in current worksheet<br />';
            ...
            } elseif (preg_match('/^'.self::CALCULATION_REGtkp_FUNCTION.'$/i', $token, $matches)) {
...
//                    echo 'Token is a number, boolean, string, null or an Excel error<br />';
                ...
                } elseif (preg_match('/^'.self::CALCULATION_REGtkp_NAMEDRANGE.'$/i', $token, $matches)) {
